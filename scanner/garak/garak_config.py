"""Configuration management for Garak integration"""
...
class GarakConfig:
    """Configuration for Garak integration"""
        ...
        if config_path and config_path.exists():
            try:
                with open(config_path, "r") as f:
                    ...
                    if file_config and "garak" in file_config:
                        config.update(file_config["garak"])
            except Exception as e:  # pragma: no cover - defensive
                logger.warning(f"Error loading Garak config from {config_path}: {e}")

        # Override with environment variables
        config["enabled"] = (
            os.getenv("GARAK_ENABLED", str(config["enabled"])).lower() == "true"
        )
        config["timeout"] = int(os.getenv("GARAK_TIMEOUT", config["timeout"]))
        config["max_concurrent"] = int(
            os.getenv("GARAK_MAX_CONCURRENT", config["max_concurrent"])
        )
        config["retry_attempts"] = int(
            os.getenv("GARAK_RETRY_ATTEMPTS", config["retry_attempts"])
        )
        config["retry_delay"] = float(
            os.getenv("GARAK_RETRY_DELAY", config["retry_delay"])
        )
        config["cache_enabled"] = (
            os.getenv("GARAK_CACHE_ENABLED", str(config["cache_enabled"])).lower()
            == "true"
        )
        config["cache_ttl"] = int(os.getenv("GARAK_CACHE_TTL", config["cache_ttl"]))
        config["rate_limit_per_minute"] = int(
            os.getenv("GARAK_RATE_LIMIT_PER_MINUTE", config["rate_limit_per_minute"])
        )
