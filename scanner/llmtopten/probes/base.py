"""Base probe class for LLMTopTen - OWASP LLM Top 10 probes"""
...
class OWASPProbe(ABC):
    """
    Base class for LLMTopTen probes (OWASP LLM Top 10) following Garak framework architecture.
    ...
    Attributes:
        ...
        owasp_id: OWASP LLM Top 10 identifier (e.g., "LLM01")
        category: Category of the probe ("owasp_llm_top10")
    """
    ...
    def __init__(self):
        ...
        self.probename = f"llmtopten.{self.__class__.__module__.split('.')[-1]}.{self.__class__.__name__}"
        ...
        logger.debug(f"Initialized LLMTopTen probe: {self.probename}")
    ...
    def test(self, model_response: str, user_query: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        ...
        This method is called by the LLMTopTen integration to evaluate
    ...
    def probe(self, generator, detector=None) -> Iterable:
        """
        Garak-style probe method using LLMTopTen generators and detectors.
        ...
        """
        from scanner.llmtopten.generators.base import Conversation, Message
...
class TextProbe(OWASPProbe):
    """
    Base class for text-based LLMTopTen probes.
    
    Most LLMTopTen probes will inherit from this class.
    """
